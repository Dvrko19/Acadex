...
class R2PrivateStorageService extends PrivateStorageService {
  constructor(options = {}) {
    super(options.rootDirectory);
    this.provider = "r2";
    this.bucket = options.bucket || process.env.R2_BUCKET;
    ...
    if (!this.bucket || !endpoint || !accessKeyId || !secretAccessKey) {
      throw new Error(
        "R2 requiere R2_BUCKET, R2_ACCOUNT_ID o R2_ENDPOINT, R2_ACCESS_KEY_ID y R2_SECRET_ACCESS_KEY"
      );
    }
  ...
  async promote(storedFileName, { filePath, mimeType } = {}) {
    if (!filePath) throw new Error("R2 promotion requires the validated local file");
    const sourceKey = this.quarantineKey(storedFileName);
    const targetKey = this.cleanKey(storedFileName);
    ...
    try {
      ...
      console.error("No se pudo limpiar el objeto de cuarentena en R2", {
        key: sourceKey,
  ...
  async createReadStream(storageKey) {
    ...
    if (!result.Body || typeof result.Body.pipe !== "function") {
      throw new Error("R2 did not return a readable object stream");
    }
    return result.Body;
  }
}
...
const createPrivateStorageService = (provider = process.env.FILE_STORAGE_PROVIDER || "local") => {
  ...
  if (selectedProvider === "r2") return new R2PrivateStorageService();
...
module.exports.R2PrivateStorageService = R2PrivateStorageService;
